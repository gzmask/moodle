...
/**
 * Strings for component 'admin', language 'zh_cn', branch 'MOODLE_20_STABLE'
 *
 * @package   admin
...
$string['bloglevelupgrade'] = '博客可见性升级';
...
$string['bloglevelupgradenotice'] = '您的网站正在使用旧的博客可见性设置，建议<a href="bloglevelupgrade.php">升级博客可见性</a>。';
...
$string['cannotdeletemissingqtype'] = '您不能删除缺失的问题类型。这是系统所需要的。
';
$string['cannotdeletemodfilter'] = '您不能卸载“{$a->filter}”，因为它是“$a->module}”模块的一部分。';
$string['cannotdeleteqtypeinuse'] = '您不能删除问题类型\'“{$a}”。题库中有这种类型的题目。
';
$string['cannotdeleteqtypeneeded'] = '您不能删除问题类型\'“{$a}”。还有其他已经安装的问题类型依赖于它。';
...
$string['configallowthemechangeonurl'] = '如果启用，那么向任何Moodle网址添加theme={themename}参数都能改变主题。';
...
$string['configcachetype'] = '选择一种Moodle使用的缓存。这个选项仅仅是配置缓存，不要忘记，必须先激活，缓存才能生效。<strong>仅仅</strong>在需要减少数据库负载的情况下使用，否则Moodle将运行得更慢。中等访问量的网站推荐使用“内部”。安装了eAccelerator或者Turckmmcache，且<em>激活共享内存选项</em>的单个web服务器应该尝试使用‘eaccelerator’。如果您有多台服务器，并且运行着一个或多个memcache守护进程和PHP-memcached扩展，选择“memcached”并且配置下面的memcached选项。<br /><strong>注意:</strong>一定要在带有负载的情况下测试性能——缓存会让您的网站更慢。在高访问量的网站中，eAccelerator和memcached能发挥最大功效，但会消耗web服务器更多的CPU。';
...
$string['configdebugstringids'] = '此选项是为方便翻译字符串而设。它在输出的字符串旁边显示对应的语言文件和字符串id。（修改此设置只会影响下一次页面加载。）';
...
$string['configenableajax'] = '此项设置允许您控制整个网站内AJAX(advanced client/server interfaces using Javascript)的使用。此设置被启用后，用户仍可以在个人资料中进行选择，否则AJAX对每个人都是禁用的。';
...
$string['configenableglobalsearch'] = '启用这个选项，允许在资源和活动中进行全文搜索，但是它不和PHP4兼容。';
...
$string['configexcludeoldflashclients'] = '某些版本的Adobe Flash插件有可被恶意Flash攻击的漏洞。您可以在这里指定支持的最低版本。Moodle不会向使用更低版本的Flash的用户显示Flash文件。他们会看到一个讲述如何升级的flash。此处留空会禁用所有的检查。';
...
$string['configextrauserselectorfields'] = '在搜索用户时（比如分配角色或向小组添加用户），除了全名外，还有哪些字段也被搜索和显示。安全起见，建议<em>不</em>要选择用户名字段。';
...
$string['configfullnamedisplay'] = '这里定义如何显示全名。对于中文网站来说，最符合习惯的设置是“语言”，意思是让当前的语言包来决定。当然，也可以选择隐藏姓或者别的。';
$string['configgdversion'] = '指明已安装的GD版本。默认显示的版本号是自动探测的结果。不要轻易改变它，除非您清楚您在做什么。';
$string['configgeoipfile'] = 'GeoIP City二进制文件位置。该文件不是Moodle发行版的一部分，需要单独从从<a href="http://www.maxmind.com/">MaxMind</a>获取。您可以购买商业版本或者使用免费版。<br />从<a href="http://www.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz" >http://www.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz</a>下载并解压到服务器的"{$a}"目录中。';
$string['configgetremoteaddrconf'] = '如果您的服务器在一个反向代理后，您可以设定此项以确定哪条 HTTP 头信息包含了访问者的 IP 地址。头信息按顺序读入，第一条出现的将被使用。';
$string['configgooglemapkey'] = '为了使用Google Maps查看具体IP地址分布，您需要输入一个专用的Key。可以通过<a href="http://code.google.com/apis/maps/signup.html" >http://code.google.com/apis/maps/signup.html</a><br />获取。您的web站点的地址是： {$a}';
...
$string['confignavshowallcourses'] = '设置此选项将在导航中永远显示网站的所有课程。';
$string['confignavshowcategories'] = '在导航条和导航版块中显示课程分类。学生已经选课的课程不受此设置影响，它们仍会被直接列在“我的课程”之下，与分类无关。';
$string['confignoreplyaddress'] = '有时email会以用户身份发送(如讨论区帖子)。有时用户不希望别人看到自己的email地址，在这些情况下，您在此处指定的email地址将被用做”发件人“地址，这样收件人就不能直接回复到用户。';
...
$string['configrestrictmodulesfor'] = '哪些课程会受到禁用某些活动模块<b>设置</b>的影响？注意，此设置仅对教师有效。管理员总是可以向课程添加任意活动。';
...
$string['configsmtphosts'] = '填入一个或多个本地SMTP服务器全名(例如“mail.a.com”或“mail.a.com;mail.b.com”)，本系统将用它(们)发送邮件。使用[server]:[port]的形式（例如“mail.a.com:587”）指定一个非标准端口（即25以外的端口）。如果留空不填，Moodle将使用PHP默认的方法发信。';
...
$string['configuseexternalyui'] = '使用Yahoo服务器上的在线文件，而不是本地文件。警告：必须要能访问互联网，否则您网站上的AJAX会失效。';
...
$string['enablecourseajax'] = '启用AJAX课程编辑器';
$string['enablecourseajax_desc'] = '编辑课程主页时可以使用AJAX。注意，课程格式和主题风格必须支持AJAX编辑，并且用户必须在个人资料中启用AJAX。';
$string['enablecourserequests'] = '可以课程申请';
$string['enableglobalsearch'] = '启用全局搜索';
$string['enablegroupmembersonly'] = '只有组内成员可用';
$string['enablehtmlpurifier'] = '启用HTML净化器';
...
$string['extrauserselectorfields'] = '当选择用户时，搜索和显示';
...
$string['invalidlangpack'] = '要编辑此语言包，您需要把它放在语言文件夹中。';
...
$string['langimportdisabled'] = '语言导入功能已被禁止。您必须手工在文件系统上更新语言包。';
...
$string['maturitycorewarning'] = '您正要安装或升级一个标记为“{$a}”的Moodle版本，它还不适合在正式网站使用。请确认您使用的Moodle版本正确且确实要这么做。';
...
$string['unicodeupgradenotice'] = '在 Moodle 1.6 中，我们已经将所有的语言迁移到 Unicode 编码。要完成这个站点的升级您需要使用迁移脚本将您的数据库中的数据全部转换为 Unicode (UTF-8)。<a href="utfdbmigrate.php">点击这里以启动迁移脚本</a>！';
